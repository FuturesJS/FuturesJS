(function () {
  ...
  function promisify_subscription(s) {
    if (!s || !s.subscribe) {
      throw Error("Not a subscription");
    }
    if (s.when) {
      return s;
    }
  ...
  function join_promises (promises, params) {
    ...
    promises.forEach(function (p, i, arr) {
      // handle subscriptions
      if (p && p.subscribe && !p.when) {
        // Do I even need to pass this back?
  ...
  function make_subscription() {
    ...
    function enroll(deed, callback) {
      ...
      if (subscribers[token]) {
        throw Error('Impossible Error: Duplicate token!');
      }
    ...
    function subscribe(callback, errback) {
      // TODO create global-ish no-op
      var unsub = function () {},
      unmis = function () {};
      
      if (!callback && !errback) {
        // TODO should be leneint and just ignore? Nah... the user should check
        // that he has an actual function to pass in. Silent errors are bad.
        throw Error('Must subscribe with either callback or errback');
      }
      if (callback) {
